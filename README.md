...
As an alternative Docker can be use to achieve the same goal. You only need to have docker installed. Docker image provide NodeJS and NPM.
* Docker (v19.03.13)

# Docker
Standing in the project run start_local.sh script which is located in the project root folder. This will build and create an image called react-elc-challenge:dev.

    sh ./start_local.sh

After doing that the script will run the built image and mount the application on a volume and expose the ports 3035 and 3030.
The app will be available on your browser on localhost:3030 and the API will be available on localhost:3035.
...
In order to change the API host and port it can be done in 'server/app.js'. There are two variables in order to do that: host and port. If this two are changed please change it as well on constants file 'scripts/constants.js'. The can be found with the same name.
